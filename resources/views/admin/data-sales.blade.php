...
<div class="content-header">
        ...
        <div class="card mb-4 shadow-sm">
            <div class="card-body d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h5 class="card-title text-dark mb-2">Total Data Transaksi Sales</h5>
                        <p class="card-text text-muted">Jumlah total data transaksi dan total netto yang tercatat dalam sistem.</p>
                    </div>
                </div>
                <div class="text-center">
                    <h3 class="font-weight-bold">{{ $query_current_sales->total() }}</h3> <!-- Jumlah transaksi -->
                    <p class="text-success">Data tersedia dalam <strong>{{ $query_current_sales->lastPage() }}</strong> halaman.</p>
                    <p class="text-dark font-weight-bold mt-2">Total Netto Hari Ini: <span class="text-primary">{{ number_format($total_sales_tonase, 2) }} Ton</span></p>
                    <small class="text-muted mt-2">* Total netto ini dihitung berdasarkan transaksi yang telah terekam hingga saat ini dan sudah melakukan regist-out.</small>
                </div>
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Data Sales (Penjualan Langsung)</h3>

            <div class="card-tools">
                <ul class="pagination pagination-sm float-right">
                    <!-- Previous Page Link -->
                    @if ($query_current_sales->onFirstPage())
                    <li class="page-item disabled">
                        <a class="page-link" href="#" tabindex="-1" aria-disabled="true">&laquo;</a>
                    </li>
                    @else
                    <li class="page-item">
                        <a class="page-link" href="{{ $query_current_sales->previousPageUrl() }}">&laquo;</a>
                    </li>
                    @endif

                    <!-- Page Number Links -->
                    @foreach ($query_current_sales->getUrlRange(1, $query_current_sales->lastPage()) as $page => $url)
                    <li class="page-item {{ $page == $query_current_sales->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                    </li>
                    @endforeach

                    <!-- Next Page Link -->
                    @if ($query_current_sales->hasMorePages())
                    <li class="page-item">
                        <a class="page-link" href="{{ $query_current_sales->nextPageUrl() }}">&raquo;</a>
                    </li>
                    @else
                    <li class="page-item disabled">
        ...
        <div class="card-body table-responsive p-0">
            <table class="table table-hover text-nowrap">
                ...
                <tbody>
                    @foreach ($query_current_sales as $index => $data )
                    <tr>
                        <td>{{ $index + 1 + (($query_current_sales->currentPage() - 1) * 5) }}.</td> <!-- Perhitungan nomor urut -->
                        <td>{{ $data->wbsid }}</td>
                        ...
                        <td>
                            ...
                            <span class="badge bg-success">Reg-out</span>
                            @else
                            <span class="badge bg-secondary">No Status</span> <!-- Default jika tidak ada status -->
                            @endif
                            @endif
                        </td>
                    </tr>
